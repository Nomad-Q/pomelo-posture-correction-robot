...
def move2Target(robot, x, y, z, angle_final, ser):
    #移动到初始位置
    ser.write(b'c;')
    pos = (-0.118242, 0.229553, 0.366960)
    rpy_xyz = (170, -11, -46.83)
    result = robot.move_to_target_in_cartesian(pos, rpy_xyz)
    time.sleep(1)
    ...
    rpy_xyz = (180, 0, angle_final)  # 这个是指 绕x，y，z轴转多少度（逆时针是+，顺时针是-）。
    result = robot.move_to_target_in_cartesian(pos, rpy_xyz)
    ser.write(b'o;')
    time.sleep(1)

    # 轴动到目标位置
    pos = (-0.094094, 0.551664, 0.112089)
    rpy_xyz = (180, 0, -50)
    result = robot.move_to_target_in_cartesian(pos, rpy_xyz)
    ser.write(b'c;')
    time.sleep(1)
